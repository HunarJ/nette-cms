...
class ArticleManager extends DatabaseManager
{
    ...
    public function getArticle(string $url, string $columns = NULL)
    {
    ...
    public function getArticleFromId(int $id, string $columns = NULL)
    {
        return $this->database->table(self::TABLE_NAME)
            ->select($columns ? $columns : '*')
    ...
    public function saveArticle(array $values)
    {
        $articleData = [
            'title' => $values['title'],
            'url' => $values['url'],
