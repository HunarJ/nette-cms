...
class CmsManager extends DatabaseManager
{
    ...
    public function getCmsPage(string $url, string $columns = null)
    {
        return $this->database->table(self::TABLE_NAME)->select($columns ? $columns : '*')->where(self::COLUMN_URL, $url)->fetch();
    }

    public function getCmsPageFromId(int $id, string $columns = null)
    {
        return $this->database->table(self::TABLE_NAME)->select($columns ? $columns : '*')->where(self::COLUMN_ID, $id)->fetch();
    }
    ...
    public function saveCmsPage(array $values)
    {
        $cmsData = [
            'title' => $values['title'],
            'url' => $values['url'],
        ...
        if (!empty($values['id'])) {
            $cms = $this->database->table('cms')->wherePrimary($values['id']);
            $cms->update($cmsData);
            $cms = $cms->fetch();
        } else {
            $cms = $this->database->table('cms')->insert($cmsData);
        }
